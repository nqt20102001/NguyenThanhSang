...
function themPrc() {
    ...
    productsList.push({ id: `${add_id}`, name: `${add_name}`, price: `${add_price}`, img: `${add_img_m}`, img: `${add_img_2}`, img: `${add_img_3}`, img: `${add_img_4}` })
    localStorage.setItem('productsList', JSON.stringify(productsList))

    if (add_id === "ts") {
        tshitsList.push({ id: `${add_id}`, name: `${add_name}`, price: `${add_price}`, img: `${add_img_m}`, img: `${add_img_2}`, img: `${add_img_3}`, img: `${add_img_4}` })
        localStorage.setItem('tshitsList', JSON.stringify(tshitsList))
    }
    else if (add_id === "hd") {
        hoodiesList.push({ id: `${add_id}`, name: `${add_name}`, price: `${add_price}`, img: `${add_img_m}`, img: `${add_img_2}`, img: `${add_img_3}`, img: `${add_img_4}` })
        localStorage.setItem('hoodiesList', JSON.stringify(hoodiesList))
    }
    else if (add_id === "st") {
        sweatersList.push({ id: `${add_id}`, name: `${add_name}`, price: `${add_price}`, img: `${add_img_m}`, img: `${add_img_2}`, img: `${add_img_3}`, img: `${add_img_4}` })
        localStorage.setItem('sweatersList', JSON.stringify(sweatersList))
    }
...
function suaPrc(x) {
    ...
    productsList.forEach(product => {
        ...
        suaPrc2.addEventListener("click", function (e) {
            alert("tao làm chưa xong đừng có phá")
            let item = [{ id: `${product.id}`, name: `${product.name}`, price: `${product.price}`, img: `${add_img_m}`, img: `${add_img_2}`, img: `${add_img_3}`, img: `${add_img_4}` }]
            productsList.splice(i, 1, item)

            if (product.id === "ts") {
                tshitsList.splice(i, 1, item)
            }
            else if (product.id === "hd") {
                hoodiesList.splice(i, 1, item)
            }
            else if (product.id === "st") {
                sweatersList.splice(i, 1, item)
            }
        })
    })
    shop()
